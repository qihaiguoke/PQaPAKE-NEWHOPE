...
void pqpake_assert_constants(void) {
  ...
  assert(PQPAKE_CT_SIZE == CRYPTO_CIPHERTEXTBYTES &&
         "ciphertext size mismatch");
  assert(PQPAKE_PK_SIZE == CRYPTO_PUBLICKEYBYTES &&
         "public key size mismatch");
  assert(PQPAKE_SK_SIZE == CRYPTO_SECRETKEYBYTES &&
         "secret key size mismatch");
  assert(PQPAKE_ECT_SIZE == CRYPTO_CIPHERTEXTBYTES &&
