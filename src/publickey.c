...
int pqpake_ic_publickey_encrypt(const uint8_t* sym_key,
  ...
  for (int i = 0; i < ROUNDS; i++) {
    ...
    uint8_t tem_tail=temp_epk[PQPAKE_ENCODEDPK_SIZE-1];
    temp_epk[PQPAKE_ENCODEDPK_SIZE-1]&= (0xff >> (PQPAKE_PADBITS));
    int is_in_range = !pqpake_ic_value_is_not_in_range(temp_epk);
    temp_epk[PQPAKE_ENCODEDPK_SIZE-1]=tem_tail;

    int epk_mask = (~finished_encryption & is_in_range) ? 0x00 : 0xff;
    for (int j = 0; j < PQPAKE_EPK_SIZE; j++) {
      epk[j] = (epk[j] & epk_mask) | (temp_epk[j] & ~epk_mask);
    }

    finished_encryption |= is_in_range;
    if(finished_encryption) break;
  }
...
int pqpake_ic_publickey_decrypt(const uint8_t* sym_key,
  ...
  for (int i = 0; i < ROUNDS; i++) {
    ...
    uint8_t tem_tail=temp_pk[PQPAKE_ENCODEDPK_SIZE-1];
    temp_pk[PQPAKE_ENCODEDPK_SIZE-1]&= (0xff >> (PQPAKE_PADBITS));
    int is_in_range = !pqpake_ic_value_is_not_in_range(temp_pk);
    temp_pk[PQPAKE_ENCODEDPK_SIZE-1]=tem_tail;

    int pk_mask = (~finished_encryption & is_in_range) ? 0x00 : 0xff;
