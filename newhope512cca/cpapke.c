                    ...
                    unsigned char *sk)
{
  ...
  poly_tomont(&ahat_shat); // added here!
  poly_add(&bhat, &ehat, &ahat_shat);

  poly_tobytes(sk, &shat);
#ifdef MY_DEBUG
  encode_pk(pk, &ahat_shat, publicseed); // without adding ehat
#else
...
void cpapke_enc(unsigned char *c,
    ...
    const unsigned char *coin)
{
    poly rhat, ep, v, ahat, bhat, epp, k;
    unsigned char publicseed[NEWHOPE_SYMBYTES];
    poly vprime, uhat, tmp, shat; // dec

    //poly_frommsg(&k, m);

    decode_pk(&bhat, publicseed, pk);
    gen_a(&ahat, publicseed);
    poly_frombytes(&shat, sk); // dec

    poly_sample(&rhat, coin, 0);
    poly_sample(&ep, coin, 1);
    poly_sample(&epp, coin, 2);

    poly_ntt(&rhat);

    poly_mul_pointwise(&uhat, &ahat, &rhat); // -1, b' = a^ (*) r^
    poly_tomont(&uhat); // 0, 

    poly_mul_pointwise(&tmp, &shat, &uhat); //  -1, tmp = s^ (*) b' = sar
    poly_invntt(&tmp); // 0
    poly_reduce(&tmp);

    poly_mul_pointwise(&v, &bhat, &rhat); //-1, v' = b^ (*) r^ = asr
    poly_invntt(&v); // 0
    poly_reduce(&v);

    // dec
    poly_sub(&tmp, &tmp, &v); // should: tmp = s(ar) == (ar)s = v
...
void cpapke_enc(unsigned char *c,
    ...
    const unsigned char *coin)
{
    poly rhat, ep, v, ahat, bhat, epp, uhat, k;
    unsigned char publicseed[NEWHOPE_SYMBYTES];
    ...
    poly_mul_pointwise(&uhat, &ahat, &rhat); // -1
    poly_tomont(&uhat); // 0, 

    poly_add(&uhat, &uhat, &ep);

    poly_mul_pointwise(&v, &bhat, &rhat);
    poly_invntt(&v);

    poly_add(&v, &v, &epp);
    poly_add(&v, &v, &k); // add message
                ...
                const unsigned char *sk)
{
  ...
  poly_mul_pointwise(&tmp, &shat, &uhat); // -1
  poly_invntt(&tmp); // 0
